...
public class BScreen implements Screen, GestureDetector.GestureListener {
    final Main game;
    ...
    public void render(float delta) {
        ...
        game.batch.begin();
        game.font.draw(game.batch, "A", (float) Gdx.graphics.getWidth() / 2, (float) Gdx.graphics.getHeight() / 2);
        game.batch.end();
    }
    ...
    private void handleClick() {
        if (Gdx.input.justTouched()) {
            ...
            if (bButtonBounds.contains(touchX, touchY)) {
                game.setScreen(new AScreen(game));
            }
        }
    }
